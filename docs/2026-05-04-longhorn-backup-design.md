...
- The cluster's gitops repo previously pinned `nebari-data-science-pack` to commit `a11f1cf`, which predates the shared-storage spawner wiring shipped in https://github.com/nebari-dev/nebari-data-science-pack/pull/30. At that pin the chart created the `shared-storage` PVC but did not mount it into singleuser pods (`/shared` was empty). Resolved by bumping the gitops to `b255465` and setting `jupyterhub.custom.shared-storage-enabled: true` to satisfy the chart's helpers-guard. The chart at HEAD wires `volume_mounts` per-group at `/shared/<group>` via the spawner's `pre_spawn_hook` and an init container that creates group dirs with `chmod 2775` for setgid propagation. Cluster bumped on 2026-05-05.
